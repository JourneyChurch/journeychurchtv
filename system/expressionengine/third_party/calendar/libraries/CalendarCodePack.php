 ...
 * @version		2.0
 * @filesource	calendar/libraries/CalendarCodePack.php
 */

class CalendarCodePack
{
	/**
	...
	public $langItems = array(
		...
		'missing_theme_html_exp'		=> 'There should be a folder called \'html\' inside your site\'s \'/themes/solspace_themes/calendar/%code_pack_name%\' folder. Make sure that it is in place and that it contains additional folders that represent the template groups that will be created by this code pack.',
		...
		'missing_code_pack_theme_exp'	=> 'There should be at least one theme folder inside the folder \'%code_pack_name%\' located inside \'/themes/calendar/\'. A theme is required to proceed.',
	...
	public function detectInternalCodePack()
	{
		$this->internalCodePack	= ! (
			get_class($this) === 'CalendarCodePack' &&
			//why the need for this? Well sometimes PHP was doing
			// (get_class($this) === 'CalendarCodePack') === true where
		...
		if ( ! empty($folders))
		{
			foreach ($folders as $folder)
			{
				$details = $this->getCodePackDetails(
					$this->addonsFolder . $folder . '/calendar/'
				);
	 ...
	 * /some/path/my_sweet_addon/calendar/ would return: my_sweet_addon
	 *
	 * @access	public
	...
	public function detectNameFromPath($path = '')
	{
		...
		while ($i--)
		{
			if (trim($segs[$i]) == 'calendar' AND $i !== 0)
			{
