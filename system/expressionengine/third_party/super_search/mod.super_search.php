...
class Super_search extends Module_builder_super_search
{
	...
	function _entries ( $ids = array(), $params = array() )
	{
		...
		foreach ( array( 'num' => 'limit', 'start' => 'offset' ) as $key => $val )
		{
			...
			elseif ( isset( ee()->TMPL->tagparams[ $key ] ) )
			{
			...
			elseif ( isset( $this->sess['uri'][ $val ] ) )
			{
			...
			elseif ( isset( $this->sess['uri'][ $key ] ) )
			{
	...
	function _prep_pagination_universal( $sql, $total_results, $url_suffix = '' )
	{
		...
		if ( ! empty( ee()->TMPL->tagparams[ 'limit' ] ) )
		{
			$this->limit	= ee()->TMPL->tagparams[ 'limit' ];
		}
		...
		if ( ! empty( ee()->TMPL->tagparams[ 'num' ] ) )
		{
			$this->limit	= ee()->TMPL->tagparams[ 'num' ];
		}
		...
		if ( ee()->TMPL->fetch_param('paginate_base') !== FALSE AND ee()->TMPL->fetch_param('paginate_base') != '' )
		{
			...
			{
				$paginate_base  = (preg_match("#".LD."path=(.+?)".RD."#", $paginate_base, $match)) ? ee()->functions->create_url($match['1']) : ee()->functions->create_url("SITE_INDEX");
			}
			elseif ( strpos( $paginate_base, 'http' ) === FALSE )
			{
				...
				$paginate_base	= ee()->functions->create_url( trim_slashes( $paginate_base ) );
			}
		...
		if ( isset( ee()->uri->segments[1] ) === TRUE AND strpos( $newuri, ee()->uri->segments[1] . '/' ) !== 0 AND ( ee()->TMPL->fetch_param('paginate_base') === FALSE OR ee()->TMPL->fetch_param('paginate_base') == '' ) )
		{						
			$temp[]	= ee()->uri->segments[1];
			
			if ( isset( ee()->uri->segments[2] ) === TRUE AND strpos( ee()->uri->segments[2], 'search&' ) === FALSE )
			{
				$temp[]	= ee()->uri->segments[2];
			}
